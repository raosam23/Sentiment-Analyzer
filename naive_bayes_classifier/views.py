...
def create_pie_chart(request, positive, negative, neutral):
    ...
    ax.axis('equal')
    ax.set_title(f'Sentiments for {file_name} using Naive Bayes')
    fig.patch.set_facecolor('#FAD9C1')
    ax.set_facecolor('#FAD9C1')

    image_file_name = f'{file_name}_nb_pie.png'
...
def computing_naive_bayes(request):
    file_name = request.session.get('file_name')
    ...
    csv_data = result_data_nb.to_csv(index=False)
    csv_file = ContentFile(csv_data.encode())
    sentiment_file_name = f'{file_name}_sentiments_nb_predictions.csv'
    ...
    return render(request, 'naive_bayes_classifier/sentiment_results.html', {"image_file_name" : image_file_name})
...
def naive_bayes(request)->render:
    file_name = request.session.get('file_name')
    return render(request, 'naive_bayes_classifier/naive_bayes.html', {'file_name' : file_name})

def compute_naive_bayes(request)->HttpResponse:
    return computing_naive_bayes(request)